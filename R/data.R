#' ideb_ensino_medio_brasil
#'
#' @format ideb_ensino_medio_brasil
#'
#' @source ideb_ensino_medio_brasil
'ideb_ensino_medio_brasil'
#'
#' ideb_fundamental_finais_brasil
#'
#' @format ideb_fundamental_finais_brasil
#'
#' @source ideb_fundamental_finais_brasil
'ideb_fundamental_finais_brasil'
#'
#' ideb_fundamental_iniciais_brasil
#'
#' @format ideb_fundamental_iniciais_brasil
#'
#' @source ideb_fundamental_iniciais_brasil
'ideb_fundamental_iniciais_brasil'
#'
#' ideb_ensino_medio_escolas
#'
#' @format ideb_ensino_medio_escolas
#'
#' @source ideb_ensino_medio_escolas
'ideb_ensino_medio_escolas'
#'
#' ideb_fundamental_finais_escolas
#'
#' @format ideb_fundamental_finais_escolas
#'
#' @source ideb_fundamental_finais_escolas
'ideb_fundamental_finais_escolas'
#'
#' ideb_fundamental_iniciais_escolas
#'
#' @format ideb_fundamental_iniciais_escolas
#'
#' @source ideb_fundamental_iniciais_escolas
'ideb_fundamental_iniciais_escolas'
#'
#' censo_escolar_2019_escolas
#'
#' @format censo_escolar_2019_escolas
#'
#' @source censo_escolar_2019_escolas
'censo_escolar_2019_escolas'
#'
#' censo_escolar_2019_docentes_centro_oeste
#'
#' @format censo_escolar_2019_docentes_centro_oeste
#'
#' @source censo_escolar_2019_docentes_centro_oeste
'censo_escolar_2019_docentes_centro_oeste'

